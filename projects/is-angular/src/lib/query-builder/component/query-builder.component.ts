...
export class QueryBuilderComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
    ...
    if (type === 'object') {
      ...
      if (config.entities) {
        this.entities = Object.keys(config.entities).map((value) => {
          const entity = (<EntityMap>config.entities)[value];
  ...
  getClassNames(...args: any[]): string | null {
    ...
    const classNames = args.map((id) => clsLookup[id] || (<any>this.defaultClassNames)[id]).filter((c) => !!c);
    return classNames.length > 0 ? <string>classNames.join(' ') : null;
  }
  ...
  changeField(fieldValue: string, rule: Rule): void {
    ...
    rule.operator = this.getDefaultOperator(field) || undefined;
  ...
  private checkEmptyRuleInRuleset(ruleset: RuleSet): boolean {
    if (!ruleset || !ruleset.rules || ruleset.rules.length === 0) {
      ...
      return ruleset.rules.some((item: RuleSet | Rule) => {
        if ((<RuleSet>item).rules) {
          return this.checkEmptyRuleInRuleset(<RuleSet>item);
        } else {
          return false;
        }
      });
